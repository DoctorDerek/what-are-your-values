export default function BattleActionBar({
  ...
  return (
    ...
    >
      ...
      >
        Undo <span aria-hidden="true">[Z]</span>
      </button>
      <button
        type="button"
        aria-label="Redo"
        ...
        className={historyActionClasses}
      >
        Redo <span aria-hidden="true">[Y]</span>
      </button>
      <button
        type="button"
        aria-label="Stop"
        onClick={onStop}
        className="bg-mapache-vivid-secondary-red cursor-pointer border-4 border-black px-2 py-2 text-sm font-black text-white uppercase shadow-[4px_4px_0px_0px_#000000] hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_#000000] focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-white active:translate-x-1 active:translate-y-1 active:shadow-none sm:px-5 sm:py-3 sm:text-xl"
      >
        Stop <span aria-hidden="true">[ESC]</span>
      </button>
    </nav>
  )
}
